...
var AppComponent = /** @class */ (function () {
    function AppComponent() {
        ...
        */
        this.searchContent = function (search) {
            // update the content search
            _this.contentRef.search = search;
        };
        /*
        * Helper function filter data
        */
        this.filtersChanged = function (e) {
            _this.contentRef.fetchData('&' + e.filters);
        };
    }
...
var ContentComponent = /** @class */ (function () {
    function ContentComponent(_contentService) {
        var _this = this;
        ...
        */
        this.filteredContent = function () {
            // if no search then return all
            if (!_this.search.trim())
                return _this.content;
            // else show only if title exists in search
            return _this.content.filter(function (cardContent) { return cardContent.title.toLowerCase().indexOf(_this.search.trim().toLowerCase()) > -1; });
        };
        /*
          * Helper function to fetch data
        */
        this.fetchData = function (queryStr) {
            if (queryStr === void 0) { queryStr = ''; }
            _this._contentService.getContent(queryStr)
...
var ContentService = /** @class */ (function () {
    function ContentService(http, datePipe) {
        this.http = http;
        this.datePipe = datePipe;
        this._url = 'https://content.guardianapis.com/search?api-key=test&tag=politics/politics';
    }
    ContentService.prototype.getContent = function (queryStr) {
        var _this = this;
        if (queryStr === void 0) { queryStr = ''; }
        ...
        return this.http.get(this._url + queryStr)
            ...
            if (!(response && response.response && response.response.results && response.response.results.length))
                return res;
            var dataReturned = response.response.results;
            // at this stage, im sure all data i need is there
            for (var key in dataReturned) {
                // format the date returned
                var date = dataReturned[key].webPublicationDate;
                try {
                    date = new Date(date);
                    date = _this.datePipe.transform(date, 'yyyy-MM-dd');
                }
                catch (error) {
                    console.error(error);
                }
...
var FiltersComponent = /** @class */ (function () {
    function FiltersComponent(calendar) {
        ...
        this.date = '';
        this.topics = [];
        this.onCheckboxChange = function (e, type) {
            var state = e.target.checked;
            var typeIndex = _this.topics.indexOf(type);
            if (state) {
                // add the selected to array if it doesnt exist
                if (typeIndex > -1)
                    return;
                _this.topics.push(type);
            }
            else {
                // remove the type from array if it exists
                if (typeIndex === -1)
                    return;
                _this.topics.splice(typeIndex, 1);
            }
            _this.emitFilters();
        };
        this.emitFilters = function () {
            var filters = '';
            // &q=debate%20AND%20economy
            if (_this.date)
                filters += 'from-date=' + _this.date;
            if (_this.topics.length) {
                if (filters)
                    filters += '&';
                filters += 'q=' + _this.topics.join('%20AND%20');
            }
            _this.filtersEvent.emit({ filters: filters });
        };
        this.setDate = function (date) {
            _this.date = date.year + '-' + date.month + '-' + date.day;
            _this.emitFilters();
        };
    }
